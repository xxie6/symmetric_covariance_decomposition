...
```{r}
optimize_factor <- function(R, ebnm_fn, maxiter, tol, v_init, lambda_k, R2k, n, KL, fitted_g_k_init){
  R2 <- R2k + lambda_k^2 - 2*lambda_k*as.numeric(t(v_init)%*% R %*%matrix(v_init, ncol = 1))
  resid_s2 <- estimate_resid_s2(n = n, R2 = R2)
  rank_one_KL <- 0
  curr_elbo <- -Inf
  obj_diff <- Inf
  fitted_g_k <- fitted_g_k_init
  iter <- 1
  vec_elbo_full <- NULL
  v <- v_init
  ...
  while((iter <= maxiter) && (obj_diff > tol)){
    ...
    R2 <- R2k - lambda_k^2 #I think this should be right now
    # R2 <- R2k + lambda_k^2 - 2*lambda_k*as.numeric(t(v) %*% R %*% matrix(v, ncol = 1))
...
sym_ebcovmf_backfit <- function(S, sym_ebcovmf_obj, ebnm_fn, backfit_maxiter = 100, backfit_tol = 10^(-8), optim_maxiter= 500, optim_tol = 10^(-8)){
  ...
  while((iter <= backfit_maxiter) && (obj_diff > backfit_tol)){
    ...
    for (k in 1:K){
      ...
      # optimize factor
      factor_proposed <- optimize_factor(R, ebnm_fn, optim_maxiter, optim_tol, sym_ebcovmf_obj$L_pm[,k], sym_ebcovmf_obj$lambda[k], R2k, sym_ebcovmf_obj$n, sym_ebcovmf_obj$KL[-k], symebcovmf_fit$fitted_gs[[k]])
