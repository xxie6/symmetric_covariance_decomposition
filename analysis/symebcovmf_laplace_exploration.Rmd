...
This is a plot of (part of) the progression of the ELBO:
```{r}
plot(symebcovmf_fit_backfit$backfit_vec_elbo_full[130000: length(symebcovmf_fit_backfit$backfit_vec_elbo_full)], ylab = 'ELBO')
```
...
When comparing number of backfit iterations, I'd imagine that flashier would require fewer iterations since the loadings estimate is essentially backfit twice during one backfit iteration -- once when backfitting the loadings and again when backfitting the factors.
